...
//使用该插件的应用名称，在配置文件中配置
static char APP_NAME[64] = "appName";
//日志的配置文件路径
...
void log_init(char * logConfPath){
	...
	if(getValue("APP_NAME",buffer) == 1){
		...
		memcpy(APP_NAME,buffer,strlen(buffer));
	...
	tArg *x;
	x.ts = 36;
	if(pthread_create(&t_1,NULL,moLogfile,x)){
		printf("funcA_t create failed!\n"); 
	}

	char log_tmp_path[LOGNUM]; 
	char log_tmp_path2[LOGNUM];
	memcpy(log_tmp_path,LOG_FILE_PATH,strlen(LOG_FILE_PATH));
	memcpy(log_tmp_path2,LOG_FILE_PATH,strlen(LOG_FILE_PATH));
	strcat(log_tmp_path,"YYERROR/");
	strcat(log_tmp_path2,"YYJYLS/");
